...
async def get_dashboard_view(bg_tasks: BackgroundTasks, service: CatApiService = Depends(get_api_service)):
...
async def get_dashboard_json(bg_tasks: BackgroundTasks, service: CatApiService = Depends(get_api_service)):
    return await service.get_dashboard_data(bg_tasks)


@router.get("/history")
async def get_history(limit: int = 10, service: CatApiService = Depends(get_api_service)):
